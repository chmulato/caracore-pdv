...
public interface VendaRepository extends JpaRepository<Venda, Long> {
	...
	@Query("select v from Venda v where (v.data between ?1 and ?2) and (v.vendedor = ?3)")
	public List<Venda> findByDataBetweenAndVendedor(Date dataInicial, Date dataFinal, Vendedor vendedor);

	@Query("select v from Venda v where (v.data between ?1 and ?2) and (v.vendedor.loja = ?3)")
	public List<Venda> findByDataBetweenAndLoja(Date dataInicial, Date dataFinal, Loja loja);
